...
void inject_lib(std::string const &lib_path, std::string const &logContext) {
    ...
    if (handle) {
        ...
        if (lib_path.find("gadget") == std::string::npos && lib_path.find("frida") == std::string::npos) {
            remap_lib(lib_path);
        } else {
            LOGI("Skipped memory remapping for Frida / Gadget: %s (prevent deadlock/crash)", lib_path.c_str());
        }
        return;
    }
    ...
    if (handle) {
        ...
        if (lib_path.find("gadget") == std::string::npos && lib_path.find("frida") == std::string::npos) {
            ...
            LOGI("Skipped memory remapping for Frida / Gadget (dlopen): %s (prevent deadlock/crash)", lib_path.c_str());
        }
        return;
    }
